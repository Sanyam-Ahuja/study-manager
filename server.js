...
const { Playlist } = require('pytube'); // Make sure pytube is available

// API endpoint to add lectures to an existing subject using a YouTube playlist URL
app.post('/api/subjects/:subjectId/add-lectures', authenticateToken, (req, res) => {
  const { subjectId } = req.params;
  ...
  playlist.fetch().then(() => {
    const chapterName = playlist.title; // Use the playlist title as the chapter name

    db.run(`INSERT INTO Chapters (subject_id, name) VALUES (?, ?)`, [subjectId, chapterName], function(err) {
      if (err) {
        return res.status(400).json({ error: 'Error adding chapter to the subject' });
      }

      const chapterId = this.lastID;

      playlist.video_urls.forEach((videoUrl, index) => {
        const videoTitle = playlist.video_titles[index];
        const videoDuration = playlist.video_durations[index]; // Duration in seconds

        db.run(`INSERT INTO Lectures (chapter_id, user_id, name, file_path, watched, duration) VALUES (?, ?, ?, ?, ?, ?)`,
          [chapterId, req.user.id, videoTitle, videoUrl, false, videoDuration]);
      });

      res.json({ message: 'Lectures added successfully' });
    });
  }).catch(error => {
    console.error('Error fetching playlist:', error);
    res.status(500).json({ error: 'Error processing the YouTube playlist' });
  });
});
